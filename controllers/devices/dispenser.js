...
exports.UpdateDispenserTemplate = asynHandler(async (req, res, next) => {
    ...
    const ServiceConditions = [
        { column: 'template_id', operator: '=', value: payload?.template_id },
    ];
    ...
    if (template_type === "video") {
        ...
        // Handle case where template_type is neither "video" nor "image"
        return sendResponse(res, 0, 200, "Sorry, you can only update image and video", [])
...
exports.UpdateDispenserCarouselTemplate = asynHandler(async (req, res, next) => {
    ...
    const columnsToSelect = []; // Use string values for column names
    const ServiceConditions = [
        { column: 'content_id', operator: '=', value: payload?.content_id },
    ];
    let temp_results = await GlobalModel.Finder(tableName, columnsToSelect, ServiceConditions)
    if (temp_results.rows.length == 0) {
        return sendResponse(res, 0, 200, "Sorry, No Record Found", [])
    }
    //oldrecord
    let background_image = req.files?.background_image
